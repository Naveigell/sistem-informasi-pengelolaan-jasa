<template>
    <div>
        <div class="modal-delete-container" v-if="technician !== null">
                        ...
                        <div>
                            ...
                            <div class="delete-messages">
                                ...
                                <span class="sub-message">
                                    <b style="font-weight: bold; font-size: 16px;">{{ technician.username }}</b> akan dihapus secara permanen.
                                </span>
                            </div>
                        </div>
                    </div>
                    <div style="padding-left: 7px; padding-right: 7px;">
                        <span class="text-verify">Tulis <b style="color: #000">"{{ technician.username }}"</b> untuk mengkonfirmasi.</span>
                        <input class="input-verify" type="text" @keyup="($event) => {button.disabled = $event.target.value !== technician.username}">
                    </div>
...
export default {
    ...
    methods: {
        ...
        onDelete(){
            ...
            const self = this;
            this.$api.delete(this.$endpoints.technicians.delete + `/${this.technician.id}`).then(function (response) {
