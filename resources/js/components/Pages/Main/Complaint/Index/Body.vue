                            ...
                            <table class="table table-inbox table-hover">
                                <tbody>
                                    <tr class="" v-for="(complaint, index) in complaints" @click="moveInto(complaint.id, $event)">
                                        ...
                                        <td class="view-message dont-show">{{ $store.state.user.data.role === 'user' ? "Kepada: Teknisi" : complaint.user.name }} <span v-if="complaint.disetujui_admin === 0" class="label pull-right" v-bind:class="{'label-danger': complaint.dikerjakan_teknisi === 0 && complaint.disetujui_user === 0, 'label-info': complaint.dikerjakan_teknisi === 1 && complaint.disetujui_user === 0, 'label-success': complaint.disetujui_user === 1 && complaint.dikerjakan_teknisi === 1 && complaint.disetujui_admin === 0}">{{ complaint.disetujui_user === 1 ? "Disetujui user" : complaint.dikerjakan_teknisi === 1 ? "Selesai dikerjakan" : "complain" }}</span></td>
