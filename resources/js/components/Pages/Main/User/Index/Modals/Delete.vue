<template>
    <div>
        <div class="modal-delete-container" v-if="user !== null">
                        ...
                        <div>
                            ...
                            <div class="delete-messages">
                                ...
                                <span class="sub-message">
                                    <b style="font-weight: bold; font-size: 16px;">{{ user.username }}</b> akan dihapus secara permanen.
                                </span>
                            </div>
                        </div>
                    </div>
                    <div style="padding-left: 7px; padding-right: 7px;">
                        <span class="text-verify">Tulis <b style="color: #000">"{{ user.username }}"</b> untuk mengkonfirmasi.</span>
                        <input class="input-verify" type="text" @keyup="($event) => {button.disabled = $event.target.value !== user.username}">
                    </div>
...
export default {
    ...
    methods: {
        ...
        onDelete(){
            ...
            const self = this;
            this.$api.delete(this.$endpoints.users.delete + `/${this.user.id}`).then(function (response) {
                self.emitToParent("success", "Hapus user berhasil", true, true);
            }).catch(function (error) {
                self.emitToParent("failed", "Hapus user gagal", false, false);
            });
        },
